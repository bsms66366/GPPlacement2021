...
import { ScrollView, StyleSheet, Text, TextInput, View, Button, FlatList, props, Image, postalCode, navaigation
 } from 'react-native';
...
const db = SQLite.openDatabase('coursesdb.db');
...
export default function App() {
  ...
  useEffect(() => {
    db.transaction(tx => {
      ...
      tx.executeSql('CREATE TABLE IF NOT EXSITS courses (id integer primary key not null, credits int, title text, address text );');
    });
    updateList();    
  }, []);
...
/* address.navigate('address', { item: address });
 const AddressLoc = ({ route }) => {
...
{/*  <View style={styles.Logo}>
      ...
      <TextInput placeholder='Surgery Name' style={{marginTop: 10, fontSize: 18, width: 250, borderColor: '#FAD607', borderWidth: 1, color: '#FAD607'}}
        onChangeText={(title) => setTitle(title)}
        value={title}/>  
       <TextInput placeholder='Surgery Address' style={{marginTop: 10, fontSize: 18, width: 250, borderColor: '#FAD607', borderWidth: 1, color: '#FAD607'}}
        onChangeText={(address) => setAddress(address)}
        value={displayCurrentAddress}/> 
      <TextInput placeholder='date' keyboardType="numeric" style={{ marginTop: 5, marginBottom: 5,  fontSize:18, width: 250, borderColor: '#FAD607', borderWidth: 1, color: '#FAD607'}}
      ...
      <FlatList 
        ...
        renderItem={({item}) => <View style={styles.listcontainer}><Text style={{fontSize: 18}}>{item.title}, {item.credits}, {item.address}</Text>
